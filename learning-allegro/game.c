...
void sprites_init()
{
    sprites._sheet = al_load_bitmap("./media/spritesheet.png");
...
void audio_init()
{
    ...
    sample_shot = al_load_sample("./media/shot.flac");
    must_init(sample_shot, "shot sample");

    sample_explode[0] = al_load_sample("./media/explode1.flac");
    must_init(sample_explode[0], "explode[0] sample");
    sample_explode[1] = al_load_sample("./media/explode2.flac");
...
void ship_init() {
    ship.x = (BUFFER_W - SHIP_W*2) - (SHIP_W / 2);
    ship.y = (BUFFER_H - SHIP_H*2) - (SHIP_H / 2);
    ship.shot_timer = 0;
    ship.lives = 3;
