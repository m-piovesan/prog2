...
void sprites_init() {
    ...
    sprites2._sheet = al_load_bitmap("ship3.png");
    sprites3._sheet = al_load_bitmap("obstacle.png");
...
bool shots_add(bool ship, bool straight, int x, int y) {
    ...
    for(int i = 0; i < SHOTS_N; i++) {
        ...
        shots[i].frame = 0;
        shots[i].used = true; // pegar isso aqui pra dizer se o tiro ainda tá em movimento ou não
...
void aliens_init() {
    for(int i = 0; i < ALIENS_ROWS; i++) {
        for(int j = i*ALIENS_COLS; j < ALIENS_COLS*(i+1); j++) {
            ...
            else aliens[j].type = ALIEN_TYPE_THICCBOI;
